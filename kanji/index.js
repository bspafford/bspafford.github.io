...
for (let i = 0; i < list.length; i++) {
    ...
    let id = i / 2;
    button.id = id;
    button.onclick = function() {click(id)};
    button.addEventListener("contextmenu", function (event) {
        event.preventDefault()
        navigator.clipboard.writeText(list[i]).then(() => {
            //alert("Copied to clipboard: " + list[i]);
            console.log("copied to clipboard: " + list[i]);
        })
    })
...
function click(id) {
    ...
    button.style.backgroundColor = colorList[knowList[id]];

    localStorage.setItem("knowList", JSON.stringify(knowList));
...
function updateButtons() {
    for (let i = 0; i < knowList.length; i++) {
        ...
        button.style.backgroundColor = colorList[knowList[i]];
    }
}
...
function updateScore() {
    ...
    for (let i = 0; i < knowList.length; i++) {
        ...
        if (knowList[i] == 1)
            yellow++;
        else if (knowList[i] == 2)
            green++;
    }

    let text = document.getElementById("num");
    text.innerHTML = "<span style='color: yellow;'>" + yellow + "</span>/" + "<span style='color: green;'>" + green + "</span>/" + knowList.length;
}
